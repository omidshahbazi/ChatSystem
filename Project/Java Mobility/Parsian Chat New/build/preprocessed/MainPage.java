...
public class MainPage extends Thread implements CommandListener
{
    ...
    public void OnNewCommandRecieved(String Content)
    {
        ...
        //
        if(str[0].equals(CommandType.GET_RELATED_MEMBERS))
        {
            try {
    ...
    {
        cgFriends.deleteAll();
        base.client.Send(CommandType.GET_RELATED_MEMBERS);
    }
...
//                cgFriends.append(response, null);
                if(response.length() > 0)
                {
                    ...
                    //
                    if(str[0].trim().equals(CommandType.GET_RELATED_MEMBERS))
                    {
                        Image status = Image.createImage("/Offline.png");
                        //
                    ...
                    else if(str[0].trim().equals(CommandType.LOGIN_FAILED))
                    {

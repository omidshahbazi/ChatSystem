...
public class Login implements CommandListener
{
    ...
    public Login(BSCoChat Base)
    {
        ...
        //
        tfUsername = new TextField("نام کاربری", "", 50, TextField.ANY);
        tfPassword = new TextField("کلمه عبور", "", 50, TextField.PASSWORD);
        //
        loginCommand = new Command("ورود", Command.SCREEN, 1);
                ...
                while (((c = BSCoChat.client.Input.read()) != '\n') && (c > -1) && ListenerRunnig)
                {
                    ...
                    if(c == 0 && sb.toString().length() > 0)
                    {
                        ...
                        if(Integer.parseInt(resp) == CommandType.LOGIN_SUCCESSFUL)
                        {
                            Stop();
                            //
                            BSCoChat.Username = tfUsername.getString();
                            BSCoChat.Password = tfPassword.getString();
                            //
    ...
    public void commandAction(Command c, Displayable s) 
    {
        if ((c == loginCommand)) 
        {
            ...
            {
                if(BSCoChat.client == null)
                {
                    BSCoChat.client = new Client();
                    Listener();
                }
                //

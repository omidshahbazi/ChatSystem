            ...
            while (true)
            {
                if (tcpCon == null || !tcpCon.Connected)
                {
                    try
                    {
                        ...
                        this.tcpCon.Connect("omid.ddns.shatel.ir", BinarySoftCo.ChatSystem.ClientDataLayer.Constants.ServerPort);
                        //
            ...
            try
            {
                while (tcpCon.Connected)
                {
                    try
                    {
                        ...
                        {
                            OnNewClientLoggedIn(new NewClientLoggedInEventArgs(int.Parse(str[1])));
                        }
                        else if (ct == CommandsType.ClientLogInFailed ||
                            ct == CommandsType.ClientLogInSuccessful)
                        {

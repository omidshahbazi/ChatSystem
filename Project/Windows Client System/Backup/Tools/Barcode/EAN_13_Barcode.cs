...
namespace BinarySoftCo.Tools.Barcode
{
    public class EAN_13_Barcode
    {
        ...
        public void DrawBarcode(Graphics Graphic, Point Point)
        {
            ...
            xPosition = xStart - Graphic.MeasureString(this.CountryCode.Substring(0, 1), font).Width;
            float yPosition = yStart + (height - fTextHeight);

            // Draw 1st digit of the country code.
            Graphic.DrawString(sTemp.Substring(0, 1), font, brush, new PointF(xPosition, yPosition));

            xPosition += (Graphic.MeasureString(sTemp.Substring(0, 1), font).Width + 43 * lineWidth) -
                (Graphic.MeasureString(sTemp.Substring(1, 6), font).Width);

            // Draw MFG Number.
            Graphic.DrawString(sTemp.Substring(1, 6), font, brush, new PointF(xPosition, yPosition));

            xPosition += Graphic.MeasureString(sTemp.Substring(1, 6), font).Width + (11 * lineWidth);

            // Draw Product ID.
            Graphic.DrawString(sTemp.Substring(7), font, brush, new PointF(xPosition, yPosition));
        ...
        private EAN_13_Barcode()
        {
            this.CountryCode = "00";
        }

        public EAN_13_Barcode(string ManufacturerCode, string ProductCode)
            : this()
        {
        ...
        public EAN_13_Barcode(string CountryCode, string ManufacturerCode, string ProductCode)
        ...
        public EAN_13_Barcode(string CountryCode, string ManufacturerCode, string ProductCode, string ChecksumDigit)
            : this(CountryCode, ManufacturerCode, ProductCode)
        {
